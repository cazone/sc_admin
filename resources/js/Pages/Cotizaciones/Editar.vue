                ...
                <div class="row g-3">
                    <div class="col-xl-12 col-lg-12 col-md-12">
                        <div class="card print_invoice p-lg-5 p-4 mb-2">
                            <div class="card-header mb-1 fs-4">Editar Cotizaci&oacute;n C-{{cotizacion.id}}</div>
...
const { clienteSelectd, loading,clientes,buscarCliente } = useBuscarCliente();
 ...
 const cotizacion = computed(() => usePage().props.value.cotizacion)
...
const terminos = ref(cotizacion.value.terminos);
onMounted( async() => {
clienteSelectd.value = cotizacion.value.cliente;
usePage().props.value.cotizacion.detalle_cotizacion.forEach(element => {
    element.url = element.url_imagen;

    element.proveedor = element.proveedores_id
    element.cantidad = parseInt(element.cantidad)
    element.isIva =  element.isIva == 1 ? true : false

   productos.push({...element});
});

})
const fecha = ref(cotizacion.value.fecha);
const form = ref({
    id: null,
    sku:'',
    descripcion: '',
...
const delProdcuto =(index)=>{
    id_delete.value.push(productos[index].id);
...
const submit = () => {
   ...
   Inertia.visit(route('cotizaciones.update', cotizacion.value.id), {
  method: 'PUT',
